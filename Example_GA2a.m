...
X_Big_Processes=randn(10000,Nper)*sigma*(dt^0.5);
X_Big_Cum=sum(X_Big_Processes,2);
...
% Initialize fitness
MeansOfPopulation=mean(X_Big_Cum(Population));
StDevsOfPopulation=std(X_Big_Cum(Population),1);
w1=10;
w2=1;
Fitness=w1*(MeansOfPopulation).^2+w2*(StDevsOfPopulation-sigma).^2;
...
subplot(1,2,1)
normplot(X_Big_Cum(Population(:,1)))
subplot(1,2,2)
histfit(X_Big_Cum(Population(:,1)),10)
figure()
plot(cumsum([zeros(100,1),X_Big_Processes(Population(:,1),:)],2)')
disp(mean(X_Big_Cum(Population(:,1))))
disp(std(X_Big_Cum(Population(:,1)),1))
